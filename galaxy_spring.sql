-- Drop objects in correct dependency order with CASCADE
DROP MATERIALIZED VIEW IF EXISTS ubigeo_peru_view CASCADE;

-- Drop tables with CASCADE to handle dependencies
DROP TABLE IF EXISTS peru_driver_escuelas CASCADE;
DROP TABLE IF EXISTS peru_centros_medicos CASCADE;
DROP TABLE IF EXISTS peru_centros_evaluacion CASCADE;
DROP TABLE IF EXISTS ubigeo_peru_distritos CASCADE;
DROP TABLE IF EXISTS ubigeo_peru_provincias CASCADE;
DROP TABLE IF EXISTS ubigeo_peru_departamentos CASCADE;

/* Peru departamentos*/
CREATE TABLE IF NOT EXISTS ubigeo_peru_departamentos (
	id_departamento varchar(2) PRIMARY KEY,
	nombre_departamento TEXT NOT NULL
	CONSTRAINT departamento_id_length
		CHECK (LENGTH (id_departamento) = 2),
	CONSTRAINT check_departamento_id
		CHECK (id_departamento ~ '^[0-9]+$')
);

/* Peru provincias*/
CREATE TABLE IF NOT EXISTS ubigeo_peru_provincias (
	id_provincia varchar(4) PRIMARY KEY,
	nombre_provincia TEXT NOT NULL,
	id_departamento varchar(2)
			NOT NULL REFERENCES ubigeo_peru_departamentos(id_departamento),
	CONSTRAINT departamento_id_length
		CHECK (LENGTH (id_departamento) = 2),
	CONSTRAINT id_provincia_length
		CHECK (LENGTH (id_provincia) = 4),
	CONSTRAINT check_id_provincia
		CHECK (id_provincia ~ '^[0-9]+$')
);

/* Peru distritos*/
CREATE TABLE IF NOT EXISTS ubigeo_peru_distritos (
	id_distrito varchar(6) PRIMARY KEY,
	nombre_distrito TEXT NOT NULL,
	id_departamento varchar(2) NOT NULL
		REFERENCES ubigeo_peru_departamentos(id_departamento),
	id_provincia varchar(4) NOT NULL
		REFERENCES ubigeo_peru_provincias(id_provincia),
	CONSTRAINT departamento_id_length
		CHECK (LENGTH (id_departamento) = 2),
	CONSTRAINT id_provincia_length
		CHECK (LENGTH (id_provincia) = 4),
	CONSTRAINT id_distrito_length
		CHECK (LENGTH (id_distrito) = 6),
	CONSTRAINT check_id_distrito
		CHECK (id_distrito ~ '^[0-9]+$')
);

/* Peru Driver escuelas*/
CREATE TABLE IF NOT EXISTS peru_driver_escuelas (
	...
	email VARCHAR(255),
	phone VARCHAR(20),
    id_distrito VARCHAR(6) NOT NULL REFERENCES ubigeo_peru_distritos(id_distrito),
    
    CONSTRAINT ruc_length CHECK (LENGTH(no_ruc) = 11),
    CONSTRAINT ruc_numeric_only CHECK (no_ruc ~ '^[0-9]+$')
);

-- Peru Centros Médicosx
CREATE TABLE IF NOT EXISTS peru_centros_medicos (
	id_establecimiento SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS peru_centros_evaluacion (
	id_establecimiento SERIAL PRIMARY KEY,
	no_ruc VARCHAR(11) NOT NULL,
	nombre_centro TEXT NOT NULL,
	email VARCHAR(255),
...
-- Trigger for Peru Driver
CREATE OR REPLACE FUNCTION peru_driver_field_upper()
RETURNS TRIGGER AS $example_table$
BEGIN
    NEW.nombre_centro:= UPPER(NEW.nombre_centro);
    NEW.direccion_centro := UPPER(NEW.direccion_centro);
    RETURN NEW;
END;
$example_table$ LANGUAGE plpgsql;

CREATE TRIGGER peru_driver_field_trigger 
BEFORE INSERT OR UPDATE ON peru_driver_escuelas
FOR EACH ROW EXECUTE FUNCTION peru_driver_field_upper();

--Trigger for Peru departamentos
CREATE OR REPLACE FUNCTION peru_departamentos_field_upper()
RETURNS TRIGGER AS $example_table$
BEGIN
    NEW.nombre_departamento:= UPPER(NEW.nombre_departamento);
    RETURN NEW;
END;
$example_table$ LANGUAGE plpgsql;

CREATE TRIGGER peru_departamentos_field_trigger
BEFORE INSERT OR UPDATE ON ubigeo_peru_departamentos
FOR EACH ROW EXECUTE FUNCTION peru_departamentos_field_upper();

--Trigger for Peru provincias
CREATE OR REPLACE FUNCTION peru_provincias_field_upper()
RETURNS TRIGGER AS $example_table$
BEGIN
	NEW.nombre_provincia := UPPER(NEW.nombre_provincia);
    RETURN NEW;
END;
$example_table$ LANGUAGE plpgsql;

CREATE TRIGGER peru_provincias_field_trigger
BEFORE INSERT OR UPDATE ON ubigeo_peru_provincias
FOR EACH ROW EXECUTE FUNCTION peru_provincias_field_upper();

--Trigger for Peru provincias
CREATE OR REPLACE FUNCTION peru_distritos_field_upper()
RETURNS TRIGGER AS $example_table$
BEGIN
	NEW.nombre_distrito := UPPER(NEW.nombre_distrito);
    RETURN NEW;
END;
$example_table$ LANGUAGE plpgsql;

CREATE TRIGGER peru_distritos_field_trigger
BEFORE INSERT OR UPDATE ON ubigeo_peru_distritos
FOR EACH ROW EXECUTE FUNCTION peru_distritos_field_upper();


--Trigger for Peru centros
CREATE OR REPLACE FUNCTION peru_centros_field_upper()
RETURNS TRIGGER AS $example_table$
BEGIN
    NEW.nombre_centro:= UPPER(NEW.nombre_centro);
    NEW.direccion_centro := UPPER(NEW.direccion_centro);
...
CREATE TRIGGER peru_centros_field_trigger
BEFORE INSERT OR UPDATE ON peru_centros_medicos
FOR EACH ROW EXECUTE FUNCTION peru_centros_field_upper();

CREATE OR REPLACE TRIGGER peru_centros_field_evaluacion_trigger
BEFORE INSERT OR UPDATE ON peru_centros_evaluacion
FOR EACH ROW EXECUTE FUNCTION peru_centros_field_upper();


--INSERTING
--PERU departamentoS
INSERT INTO ubigeo_peru_departamentos (id_departamento, nombre_departamento) VALUES
('01' ,'Amazonas'),
('02' ,'ANCASH'),
('03' ,'APURIMAC'),
('04' ,'AREQUIPA'),
...
--PERU provinciaS
INSERT INTO ubigeo_peru_provincias (id_provincia, nombre_provincia, id_departamento) VALUES
('0101' ,'CHACHAPOYAS' ,'01'),
('0102' ,'BAGUA' ,'01'),
('0103' ,'BONGARA' ,'01'),
...
-- PERU distritoS
INSERT INTO ubigeo_peru_distritos (id_distrito, nombre_distrito, id_provincia, id_departamento) VALUES
...
('120609','VIZCATÁN DEL ENE','1206','12'),
...
INSERT INTO peru_driver_escuelas (no_ruc, nombre_centro, direccion_centro, estado_autorizacion, id_distrito)
VALUES
('20604309833', 'CENTRO MÉDICO AMAZONAS E.I.R.L.', 'JR. GRAU N° 205, 1ER. PISO', true, '010101'),
('20613258346', 'MANEJANDO PERU E.I.R.L.', 'JR. LIBERTAD S/N (CUADRA 01), PRIMER PISO', true, '010101'),
('20600138911', 'ESCUELA DE CONDUCTORES PROFESIONALES INTEGRALES EDMAR S.R.L.', 'JR. CIRCUNVALACIÓN N° 612 LT. 1JA V38, URB. VISALOT ', true, '010701'),
('20477238336', 'ASOCIACION GUIA MIS RUTAS', 'AV.PEDRO PABLO ATUSPARIA / JR.GABINO URIBE S/N., MZ. 8 LT. 13, BARRIO PEDREGAL MEDIO,1ER. Y 2DO. PISO', true, '020101'),
('20607082902', 'AUTO LATINO E.I.R.L.', 'AV. CONFRATERNIDAD INTERNACIONAL SUR N° 244 PRIMER PISO VILLON ALTO', true, '020101'),
('20482764151', 'VIAS SEGURAS S.A.C.', 'AV. CONFRATERNIDAD INTERNACIONAL SUR MZ. 176-B, LT.24, 1ER PISO BARRIO VILLON  ALTO', true, '020101'),
('20609522365', 'GARO CAR E.I.R.L.', 'URB. BRUCES MZ. B LT 5, 2DO PISO ', true, '021809'),
('20477599193', 'GLOBAL CAR CORP S.A.C.', 'Programa de Vivienda Sector 1A-1B, Zona 1, Urb. Buenos Aires, Mz. A, Lt. 46, Primer Piso', true, '021809'),
('20490899023', 'ESCUELA DE CONDUCTORES MI BREVET SOCIEDAD ANONIMA CERRADA - ECOMIBRE S.A.C.', 'AV. MARIÑO  117 (COST.MINISTERIO D TRANSPORT C3P) APURIMAC ABANCAY ABANCAY', true, '030101'),
('20563859246', 'ESCUELA DE CONDUCTORES SEÑOR DE LOS MILAGROS E.I.R.L.', 'Lote 10, Mz. A de la Asoc. Proviv. Luis Alberto Sanchez, Barrio de Patibamba', true, '030201'),
('20608607430', 'ESCUELA DE CONDUCTORES PRADERA DE LOS CELAJES E.I.R.L.', 'JR. LEONIDAS BERROCAL N° 110', true, '030216'),
('20495109004', 'SAN CRISTOBAL DEL PERU S.A.C.', 'JR. LEÓNIDAS BERROCAL 145', true, '030216'),
('20539698282', 'AUTOESCUELA AQP LA SALLE S.A.C.', 'AV. KENNEDY 1505', true, '040112'),
('20311650751', 'CETPRO PEDRO RUIZ GALLO S.R.L.', 'URB. CESAR VALLEJO MZ. C, LT. 14-A', true, '040112'),
('20608763733', 'ESCUELA DE CONDUCTORES EDUCACION VIAL SAC', 'MZ.  B  LT.  13, 1ER.  Y  2DO.  PISO, FUNDO  KENNEDY', true, '040112'),
('20605825673', 'ESCUELA DE CONDUCTORES INTEGRALES BREVETES ELVIS S.A.C.', 'AV. GUARDIA CIVIL 2DA FASE, III ETAPA, MZ. B, LT.13 (3ER PISO)', true, '040112'),
('20455807841', 'ESCUELA DE CONDUCTORES INTEGRALES TRANSITO LIBRE S.A.C.', 'Av. Jhon F. Kennedy 1410, 3er Piso, Urb. Jesus Maria', true, '040112'),
('20529391430', 'ESCUELA PERUANA DE CONDUCTORES INTEGRALES RUTAS SEGURAS S.A.C', 'ASOCIACIÓN CENTRO INDUSTRIAL LAS CANTERAS, MZ. O, LOTE 1', true, '040112'),
('20455114561', 'GITOCA S.R.L.', 'PUEBLO JOVEN CESAR VALLEJO MZ. D LT. 13 PARTE DEL 1ER Y 2DO PISO', true, '040112'),
('20498476539', 'SGM INGENIEROS E.I.R.L.', 'Av. John F. Kennedy N° 1410, 3er Piso, Urb. Jesus Maria', true, '040112'),
('20607478067', 'ESCUELA DE CONDUCTORES INTEGRALES CONDUCE AYACUCHO S.A.C.', 'JR. MARIANO MELGAR N° 451, URB. LAS NAZARENAS', true, '050115'),
('20534589639', 'ESCUELA DE CONDUCTORES INTEGRALES SOCIEDAD ANONIMA CERRADA BREVETE AYACUCHO S.A.C.', 'JR. MARIANO MELGAR 475', true, '050115'),
('20534789565', 'INSTITUTO DE INVESTIGACION VIAL CLUB AUTOMOVIL PERU S.A.C. - INSTIVI CLUB AUTOMOVIL PERU S.A.C.', 'Jr. Mariano Melgar N° 403, 3er Piso, Urb. Las Nazarenas', true, '050115'),
('20604034893', 'MI BREVETE SEGURO S.A.C.', 'JR.  MARIANO  MELGAR  N°  456,  URB. SIMÓN  BOLÍVAR', true, '050115'),
('20609578140', 'INSTITUTO DRIVE CAR MI PERU J & B S.A.C', 'JIRÓN GONZALES VIGIL N° 222', true, '050401'),
('20609578140', 'INSTITUTO DRIVE CAR MI PERU J & B S.A.C', 'JR. CHOYMACOTA MZ. L, LOTE 11', true, '050408'),
('20601994501', 'CENTRO DE EVALUACIONES Y CERTIFICACIONES EL CIRCUITO E.I.R.L.', 'JR. REYNA FARGE N° 100 BARRIO LA FLORIDA SECTOR 13', true, '060101'),
('20570580729', 'CONSORCIO ELTON ZENA S.A.C.', 'JUAN PABLO II 182 - 3ER PISO ', true, '060101'),
('20606871971', 'ESCUELA INTEGRAL PILOTOS CAJAMARCA S.A.C.', 'Jr. Huánuco N° 513 (sótano), barrio San Pedro ', true, '060101'),
('20491827913', 'Escuela Nacional de Conductores Integrales PERU AL VOLANTE S.A.C.', 'Jr. Cinco Esquinas N° 686 / Sector N° 01 - San Sebastian', true, '060101'),
('20610717048', 'SOBRE RUEDAS JAEN S.A.C.', 'Jr. Sara Macdougall N° 527, Zona Las Margaritas, Sector 2 ', true, '060101'),
('20539808487', 'GLOBAL CAR PERU S.A.C. (Antes: MULTISERVICIOS GLOBAL CAR S.A.C.)', 'AV. FRAY JOSE ARANA N° 881 (1ER PISO Y MEZANINE) ', true, '060401'),
('20608792857', 'ESCUELA DE CONDUCTORES SAN JOSE JAEN E.I.R.L.', 'AV.  PAKAMUROS  Nº  1699', true, '060801'),
('20612981532', 'GOL ESCUELA DE CONDUCTORES E.I.R.L.', 'CALLE SAMUEL CASTAÑEDA FARFÁN  LT. 27, MZ. G, SECTOR LA HUACA, SEGUNDO PISO', true, '060801'),
('20600560311', 'SAN CRISTOBAL VIP S.A.C.', 'AV. NÉSTOR GAMBETA MZ. A LTE.2, SEGUNDO PISO URBANIZACIÓN PROGRESIVA LAS ORQUÍDEAS 2', true, '070101'),
('20521026422', 'ESCUELA DE CONDUCTORES PROFESIONALES PERUVIAL SOCIEDAD ANONIMA CERRADA', 'AV. DE LOS COCOS CON AV. DOS MZ.I LT. 01 ', true, '070104'),
('20547272162', 'ESCUELA DE CONDUCTORES Y CAPACITACIONES EN GENERAL MARBALPERU E.I.R.L.', 'AV. NÉSTOR GAMBETA MZ. A LOTE 08, AA.HH. VIRGEN DE FÁTIMA (PISOS 4 Y 5)', true, '070106'),
('20490356241', 'ASOCIACION CIVIL ESCUELA DE CONDUCTORES LIDERES DEL SUR-LIDESUR', 'URB. CONSTANZA MZA. A LOTE. 12B (CSD IZQ INDECOPI C6P BEIGE 5TO PISO) CUSCO CUSCO WANCHAQ', true, '080108'),
('20450594483', 'CENTRO DE EDUCACION TECNICO PRODUCTIVA VIRGEN DEL TRANSITO S.C.R.L. - CETPRO VIRGEN DEL  TRANSITO SRL', 'AV. PEDRO VILCAPAZA 208 (3er Piso) ', true, '080108'),
('20490887955', 'ESCUELA DE CONDUCTORES CUSCO IMPERIAL E.I.R.L. - ESCOCUSCO E.I.R.L.', 'AV. MICAELA BASTIDAS 329 ', true, '080108'),
('20490812947', 'ESCUELA DE CONDUCTORES INTEGRALES ABC DEL CONDUCTOR S.R.L.', 'AV. MICAELA BASTIDAS 327', true, '080108'),
('20491121221', 'ESCUELA DE CONDUCTORES INTEGRALES CUSCO EN RUTA SOCIEDAD COMERCIAL DE RESPONSABILIDAD LIMITADA-ESCUE', 'AV. MICAELA BASTIDAS C.H. PACHACUTEC 329 (FRENTE  A  TRANSPORTES) CUSCO CUSCO WANCHAQ', true, '080108'),
('20601129591', 'ESCUELA DE CONDUCTORES INTEGRALES ECOSBREVET SOCIEDAD ANONIMA CERRADA', 'MZ.L1 LT.25 1 Y 2 PISO (MALECON CUSCO - JR. 14 DE OCT.) ', true, '080601'),
('20491050374', 'ESCUELA DE CONDUCTORES INTEGRALES SAN JOSE OBRERO DE SICUANI SOCIEDAD ANONIMA CERRADA', 'AV. CENTENARIO  408 (COSTADO DE I.E INMACULADA CASA VERDE) CUSCO CANCHIS SICUANI', true, '080601'),
('20491050374', 'ESCUELA DE CONDUCTORES INTEGRALES SAN JOSE OBRERO DE SICUANI SOCIEDAD ANONIMA CERRADA', 'ASOC. PRO - VIVIENDA MAGISTERIAL LOTE N° 11 MZ. D ', true, '080801'),
('20604034893', 'MI BREVETE SEGURO S.A.C.', 'JR. FRANCISCO DE ANGULO N° 493-495 ', true, '090101'),
('20600560311', 'SAN CRISTOBAL VIP S.A.C.', 'PJE. FERRUA 150 BARRIO SANTA ANA ', true, '090101'),
('20541372220', 'CENTRO DE CAPACITACION PROFESIONAL INGENIEROS S.A.C. - CCP INGENIEROS S.A.C', 'JR. PEDRO PUELLES 461', true, '100101'),
('20609402025', 'ESCUELA DE CONDUCTORES INTEGRALES G&C HUÁNUCO E.I.R.L.', 'JR. HUALLAYCO N° 1761, 4TO. PISO', true, '100101'),
('20601994501', 'CENTRO DE EVALUACIONES Y CERTIFICACIONES EL CIRCUITO E.I.R.L.', 'AV. MATÍAS MANZANILLA N° 316-A, SEGUNDO PISO', true, '110101'),
('20600553276', 'ESCUELA DE CONDUCTORES INTEGRALES VIRGEN DE CHAPI S.A.C.', 'Av. Fernando León Arechua N° 248', true, '110101'),
('20487785777', 'ESCUELA PERUANA DE CONDUCTORES PROFESIONALES SOCIEDAD ANONIMA CERRADA- ESPECONP SAC', 'Calle Luisa de La Torre N° 147, Urb. San Miguel', true, '110101'),
('20612981532', 'GOL ESCUELA DE CONDUCTORES E.I.R.L.', 'AV. FERNANDO LEÓN DE VIVERO N° 05 -SUBLOTE A-INTERIOR B', true, '110101'),
('20495109004', 'SAN CRISTOBAL DEL PERU S.A.C.', 'AV. CALIFORNIA B-9, URB. SAN MIGUEL', true, '110101'),
('20600553276', 'ESCUELA DE CONDUCTORES INTEGRALES VIRGEN DE CHAPI S.A.C.', 'CALLE LIMA N° 423', true, '110201'),
('20612981532', 'GOL ESCUELA DE CONDUCTORES E.I.R.L.', 'CALLE AURELIO MOISÉS FLORES N° 509, PRIMER PISO', true, '110201'),
('20487286002', 'ESCUELA DE CONDUCTORES INTEGRALES FABIAN´S S.R.L.', 'Prolongación Cuzco N° 248, 250 y 252, 1er y 2do piso', true, '120101'),
('20600722647', 'INSTITUTO DE FORMACION VIAL DEL PERU SOCIEDAD ANONIMA CERRADA - INFOVIAL PERU S.A.C.', 'JR. GUIDO N° 620, 1ER Y 2DO PISO', true, '120101'),
('20477238336', 'ASOCIACION GUIA MIS RUTAS', 'JR.HUMBOLDTN°210', true, '120107'),
('20541372220', 'CENTRO DE CAPACITACION PROFESIONAL INGENIEROS S.A.C. - CCP INGENIEROS S.A.C', 'CAL. REAL 694', true, '120107'),
('20568101881', 'EMPRESA DE CAPACITACION E INVERSIONES SOLORZANO S.A.C.', 'JR. RAIMONDI  102 (FTE MINISTERIO DE TRANSPORTE) JUNIN HUANCAYO CHILCA                       ', true, '120107'),
('20482833811', 'ESCUELA DE CONDUCTORES INTEGRALES ALLAIN PROST E.I.R.L.', 'JR. MARISCAL CASTILLA N° 1791 ', true, '120107'),
('20482833811', 'ESCUELA DE CONDUCTORES INTEGRALES ALLAIN PROST E.I.R.L.', 'CALLE LAS CUCARDAS LT. 04, MZ. C, 1ER PISO, AA.VV SECTOR SANTO TOMAS - LA MERCED', true, '120301'),
('20600722647', 'INSTITUTO DE FORMACION VIAL DEL PERU SOCIEDAD ANONIMA CERRADA - INFOVIAL PERU S.A.C.', 'JR. AUGUSTO HILSER N° 703', true, '120601'),
('20477238336', 'ASOCIACION GUIA MIS RUTAS', 'AV. LOS INCAS N° 154, 1ER PISO, URB. SANTA MARÍA', true, '130101'),
('20607082902', 'AUTO LATINO E.I.R.L.', 'AV. MOCHE N° 603-A', true, '130101'),
('20439076977', 'CENTRO DE GESTION CAPACITACION Y DESARROLLO EN TRANSPORTE', 'CAL. NICOLAS REBAZA 660', true, '130101'),
('20482833811', 'ESCUELA DE CONDUCTORES INTEGRALES ALLAIN PROST E.I.R.L.', 'LOS INCAS 131', true, '130101'),
('20482147110', 'ESCUELA DE CONDUCTORES INTEGRALES GRAN PILOTO E.I.RL.', 'Av. Moche Nº 475 2do Piso Urb. Chicago', true, '130101'),
('20604754764', 'ESCUELA DE CONDUCTORES INTEGRALES LIDER CAR E.I.R.L.', 'AV. MOCHE 524', true, '130101'),
('20609665077', 'ESCUELA DE CONDUCTORES INTEGRALES MUNDO CAR BREVETES E.I.R.L.', 'AV. MOCHE N°537-539 (1ER Y 2DO PISO), URB. CHICAGO', true, '130101'),
('20613915053', 'ESCUELA DE CONDUCTORES PLUTON S.A.C.', 'AV. MOCHE N° 549, SEGUNDO PISO, URB. TORRES ARAUJO', true, '130101'),
('20600213769', 'ESCUELA GLOBAL CAR S.A.C.', 'CALLE MEXICO N° 500,1ER. PISO, URB.TORRES ARAUJO ', true, '130101'),
('20609380501', 'GLOBAL CAR TRUJILLO S.A.C.', 'AV. AMÉRICA OESTE N° 578, INTERIOR N° 4A, 4TO. PISO', true, '130101'),
('20601323266', 'JR FLORIAN MEDICAL CENTER S.A.C.', 'AV. MOCHE N 429, URB. CHICAGO', true, '130101'),
('20608428578', 'RUTAS LIBERTEÑAS S.A.C.', 'AV. LOS INCAS N° 279, 2DO. Y 3ER', true, '130101'),
('20609522365', 'GARO CAR E.I.R.L.', 'CALLE BENITO JUÁREZ N° 1382 – PRIMER PISO', true, '130105'),
('20482833811', 'ESCUELA DE CONDUCTORES INTEGRALES ALLAIN PROST E.I.R.L.', 'CALLE FRANCISCO BOLOGNESI MZ. 17 LOTE N° 09 (1ER Y 2DO PISO) ', true, '130203'),
('20607143359', 'ESCUELA DE CONDUCTORES INTEGRALES PERLA DE CHEPEN S.A.C.', 'MZ. C, LT. 10, URBANIZACIÓN EL REFUGIO ', true, '130401'),
('20477412972', 'EMPRESA DE SERVICIOS MEDICOS MOVIL MEDIC PERU E.I.R.L.', 'MZ. G LOTE 93 AA.HH LOS HÉROES DE HUAMACHUCO', true, '130901'),
('20608451146', 'GRUPO CORPORATIVO SAN ISIDRO S.A.C.', 'JR.  ATAHUALPA  N°  950 –3ER(*)Y  4TO  PISO', true, '130901'),
('20614027542', 'ESCUELA DE CONDUCTORES ORION E.I.R.L.', 'SANTA ELENA I-A. U.C. N° 05196 (U.C. N° 01997-P ANTERIOR) ', true, '131201'),
('20487813576', 'ASOCIACION ESCUELA DE CONDUCTORES INTEGRALES SIPAN - AECI - SIPAN', 'AV. ELVIRA GARCIA Y GARCIA 985', true, '140101'),
('20539064861', 'ESCUELA DE CONDUCTORES INTEGRALES MILAGROSA CRUZ DE MOTUPE EIRL', 'AV.  BOLOGNESI  N°1356, 3ER.  PISO, P.J.  DIEGO  FERRE', true, '140101'),
('20609582732', 'ESCUELA DE CONDUCTORES INTEGRALES RUTA 22 EIRL', 'CALLE  TUMBES  NORTE  N°  296, 1ER.  Y  2DO.  PISO, URB. PATAZCA', true, '140101'),
('20487785777', 'ESCUELA PERUANA DE CONDUCTORES PROFESIONALES SOCIEDAD ANONIMA CERRADA- ESPECONP SAC', 'Calle Las Acacias N° 211, 1° y 2° Piso, Urb Santa Victoria', true, '140101'),
('20482764151', 'VIAS SEGURAS S.A.C.', 'AV. AUGUSTO B. LEGUÍA N° 1169, TERCER PISO', true, '140101'),
('20609464993', 'CHICLAYO VIAL CIX E.I.R.L.', 'MZ. E, LT 2, LOTIZ/HAB. URB. RICARDO ARIZOLA TIRADO', true, '140112'),
('20539808487', 'GLOBAL CAR PERU S.A.C. (Antes: MULTISERVICIOS GLOBAL CAR S.A.C.)', 'UBIC. RUR. LOTE DEL FUNDO CERROPON, 1ER PISO ', true, '140112'),
('20100451779', 'ASOC NAC DEL TRANSP TERRESTRE DE CARGA', 'AV. AREQUIPA 330, INT. 901, URB. SANTA BEATRIZ', true, '150101'),
('20613589334', 'ESCUELA DE CONDUCTORES CONDUCAR VIAL E.I.R.L.', 'ASOC. POPULAR VILLA MAR DE ANCÓN MZ.11, LT.10-A, 1ER PISO', true, '150102'),
('20608302876', 'CENTRO MEDICO PANDO S.A.C.', 'AV. JOSÉ CARLOS MARIÁTEGUI LT. 3, UCV 1, ZONA A, 1 Y 2 PISO ', true, '150103'),
('20613461214', 'CONDUCCIÓN SEGURA S.A.C.', 'LT N°21 MZ.T PROG. DE VIV. RES. SOL DE VIÑAS IV ETAPA ', true, '150103'),
('20611705981', 'SALUDVIAL E.I.R.L.', 'AV. LOS QUECHUAS N° 1127-1135, MZ. G, LT. 17 - 1ER PISO, URB. OLIMPO - 1RA ETAPA', true, '150103'),
('20606936401', 'SAN GABRIEL INVERSIONES MEDICAS S.A.C.', 'AV. LOS ÁNGELES MZ. S LOTE 14, URB. LOS ÁNGELES, 2DO PISO', true, '150103'),
('20604286736', 'SERVICIOS Y TRANSPORTES JD SAN GABRIEL S.A.C.', 'AV. METROPOLITANA MZ. B-1 LT. 42, 1ER. PISO, URBANIZACIÓN CERES, II ETAPA', true, '150103'),
('20613269879', 'VIAS CARLIER S.A.C.', 'URB. POPULAR ASOCIACIÓN DE VIVIENDA LA FLORIDA CALIFORNIA, MZ. D, LOTE 41', true, '150103'),
('20538779742', 'VSP INVERSIONES S.A.C.', 'AV.NICOLÁS AYLLON N°4192, 3ER PISO  ', true, '150103'),
('20602001483', 'ESCUELA INTEGRAL DE CONDUCTORES DE TRANSPORTE TERRESTRE JESUS S.A.C.', 'AV. ALFONSO UGARTE N° 1346, OFICINA N° 202', true, '150105'),
('20607014419', 'VIALAB PERU E.I.R.L.', 'PROLONGACIÓN LOS MANZANOS INTERIOR 316-B ', true, '150107'),
('20601963401', 'CORPORACION DANCOHUA PERU S.A.C.', 'AV. MAESTRO PERUANO 437 2DO PISO URB. CARABAYLLO II ETAPA ', true, '150110'),
('20536611062', 'ESCUELA DE CONDUCTORES LUZ VERDE S.A.C.', 'AV. TUPAC AMARU N° 3979, LT. 05 MZ. R, 1ER Y 4TO PISO ', true, '150110'),
('20604780641', 'GLOBAL MEDIC LIMA S.A.C.', 'AV. UNIVERSITARIA NORTE N° 6519, 3ER PISO, URB. SANTA LUZMILA II ETAPA', true, '150110'),
('20613256530', 'M1 PRO S.A.C.', 'AV. GERARDO UNGER N° 6349-A, TERCER PISO, URB. SANTA LUISA, PRIMERA ETAPA', true, '150110'),
('20550502870', 'ESCUELA DE CONDUCTORES INTEGRALES MARITZA A4 E.I.R.L.', 'CA. LOS NARCISOS MZ. I Lt 20, 3ER Y 4TO P, A.H. ANCIETA ALTA ', true, '150111'),
('20610384626', 'B & S LOS PROFESIONALES S.A.C.', 'AV.  LAS VIOLETAS  N°  348,  2DO  PISO.  URB.  LAS  VIOLETAS', true, '150112'),
('20612058742', 'ESCUELA DE CONDUCTORES LIMA NORTE EXPRESS S.A.C.', 'CALLE LAS AMAPOLAS N° 161, PRIMER PISO, URB. LAS VIOLETAS ', true, '150112'),
('20604034893', 'MI BREVETE SEGURO S.A.C.', 'AV. CARLOS IZAGUIRRE 108, INT. 309, PISO 3, URB. PAnombreRICANA NORTE', true, '150112'),
('20600560311', 'SAN CRISTOBAL VIP S.A.C.', 'Av. Carlos Izaguirre N° 108, interior N° 308', true, '150112'),
('20604034893', 'MI BREVETE SEGURO S.A.C.', 'AV. LA MOLINA N° 2830, MZ D, LOTE 02, INT. C-16 (TERCER NIVEL),  URB.  EL  SAUCE  DE  LA  RINCONADA', true, '150114'),
('20602607721', 'ESCUELA TRANSITO Y TRANSPORTE VIP S.A.C.', 'Av. Gerardo Unger N° 5151, 3er Y 4to Piso Urb. Industrial Naranjal Mz A Lt 4', true, '150117'),
('20611705981', 'SALUDVIAL E.I.R.L.', 'CALLE DAVID ALVA MZ. H, LT. 05, SEGUNDO PISO, URB. CAJABAMBA', true, '150117'),
('20608302876', 'CENTRO MEDICO PANDO S.A.C.', 'AV. TÚPAC AMARU MZ. O-1, LT. 3-D, SEGUNDO PISO, COMUNEROS DE JICAMARCA SECTOR  8', true, '150118'),
('20535858331', 'ESCUELA DE CAPACITACION EL VOLANTE SEGURO S.R.L. - ECEVOS S.R.L.', 'CALLE LUCANAS PARCELA B, INT. TERCER PISO, MZ. 161-B, LOTE 22, VILLA POETA, JOSÉ GÁLVEZ BARRENECHEA', true, '150123'),
('20606225688', 'BMR MULTISERVICIOS S.A.C.', 'AV. BUENOS AIRES N° 340, PISO 3, MZ. 28, LT. 34, C.P. CERCADO DE PUENTE PIEDRA', true, '150125'),
('20609480255', 'ESCUELA DE CONDUCTORES VIRGEN DEL YAUCA S.A.C.', 'AV. PUENTE PIEDRA N° 929 P. 1, ASOC. DE PROP. LOS GRAMADALES ', true, '150125'),
('20612134911', 'INTERZA E.I.R.L.', 'PUENTE PIEDRA N° 537 NIVEL 3 C.P. CERCADO DE PUENTE PIEDRA ', true, '150125'),
('20535780715', 'ASOCIACION ESCUELA NACIONAL INTEGRAL DE CONDUCTORES PROFESIONALES ALO LICENCIAS DE CONDUCIR ORGANIZA', 'AV. 9 DE OCTUBRE N°459 TRES COMPUERTAS (3° Y 4° PISO) ', true, '150132'),
('20609340518', 'EEC RUTAS DEL PERU PREMIUM S.A.C.', 'SV MZ. G, LT. 2, APV GARAGAY, 3ER PISO', true, '150132'),
('20611670851', 'ESCUELA DE CONDUCTORES EMANUEL E.I.R.L.', 'MZ. O-6 LT.40, 2DO. PISO, A.H. 5 DE NOVIEMBRE', true, '150132'),
('20611574976', 'ESCUELA DE CONDUCTORES LIDER CARD EMPRESA INDIVIDUAL DE RESPONSABILIDAD LIMITADA', 'AV. PRÓCERES DE LA INDEPENDENCIA N° 2308, MZ. A, LT. 02, EL MANTARO, TERCER PISO', true, '150132'),
('20610248056', 'ESCUELA INTEGRAL DE CONDUCTORES A.G.G E.I.R.L.', 'AV. 13 DE ENERO N° 2372, URB. SAN CARLOS ', true, '150132'),
('20613702700', 'RUTAS PERU PREMIUN E.I.R.L.', 'JIRÓN ARNALDO MARQUEZ MZ. J2, LOTE 25, 4TO PISO', true, '150132'),
('20609256983', 'BREVETES V & E S.A.C.', 'AV. PRLG. RAMON V. MACHUCA N°112 MZ. I LT.15 PISO 2 ', true, '150133'),
('20613639943', 'ESCUELA BREVETE FACIL AMERICA E.I.R.L.', 'AV. EL TRIUNFO MZ. P LOTE 22-B, PRIMER PISO ', true, '150133'),
('20612298859', 'ESCUELA CONDUFACIL AMERICA E.I.R.L.', 'COOP. DE VIVIENDA AMÉRICA MZ. A, LT. 13 Y 14, INT. A, PRIMER PISO', true, '150133'),
('20610239430', 'ESCUELA DE CONDUCTORES BREVETES CITYCARS E.I.R.L.', 'AV. ANDRÉS AVELINO CÁCERES MZ. A LOTE 7 (3ER PISO), AAHH LAS TORRES DE SAN JUAN', true, '150133'),
('20608278461', 'ESCUELA DE CONDUCTORES CONDUCAR E.I.R.L.', 'MZ . A, LOTE 13 Y 14, INTERIOR A, COOPERATIVA DE VIVIENDA AMÉRICA', true, '150133'),
('20605606017', 'ESCUELA TECNICA DE CONDUCCION Y SEGURIDAD VIAL PROFESIONALES A S.A.C.', 'CALLE EDGARDO REBAGLIATTI, MZ. H LT. 21, 1ER. PISO', true, '150133'),
('20608764977', 'ESCUELA DE CONDUCTORES INTEGRALES CARRETERAS DEL SUR SAC', 'AV. SAN JUAN N° 233-A, URB. AGRUPACIÓN ESTHER', true, '150134'),
('20477238336', 'ASOCIACION GUIA MIS RUTAS', 'AV. ALFREDO MENDIOLA 1201', true, '150135'),
('20609239418', 'ECI TRANSITO & TRANSPORTE SAC', 'AV. SANTIAGO ANTÚNEZ DE MAYOLO MZ. C, LT. 10, COOPERATIVA EL OLIVAR (3ER PISO)', true, '150135'),
('20607587192', 'ZYC CONDUCTORES VIAL PREMIUM E.I.R.L.', 'AV. UNIVERSITARIA N° 3851, PISO 1,URB. HUAYTAPALLANA', true, '150135'),
('20613199722', 'ESCUELA BREVETEFACIL PERU E.I.R.L.', 'AV. LOS CIPRESES 336 URB. LOS FICUS 1ER PISO', true, '150137'),
('20613680455', 'CALEM II E.I.R.L.', 'MZ. E, LOTE 08, 2DO PISO, COOP. VIRGEN DE COCHARCAS', true, '150142'),
('20606934824', 'CALEM PERU E.I.R.L.', 'COOP. VIRGEN DE COCHARCAS MZ. E, LT. 14', true, '150142'),
('20606602716', 'INVERSIONES VIALES SEGURIDAD TOTAL S.A.C.', 'SECTOR 2, GRUPO 24, MZ. B, LT. 10, 3ER Y 4TO PISO', true, '150142'),
('20602190359', 'LA ESCUELA T APRUEBO - V SOCIEDAD ANONIMA CERRADA - LA ESCUELA T APRUEBO - V S.A.C.', 'AV. PAnombreRICANA SUR KM 21  MZ. B LT. 5 ,2DO PISO, URB.  VILLA EL  MILAGRO –CONCHAN', true, '150142'),
('20604627819', 'MEDICAL CENTER DEL PERU S.A.C.', 'MZ. O LT. 5, GRUPO RESIDENCIAL 8, SECTOR 2, 3ER. PISO', true, '150142'),
('20611705981', 'SALUDVIAL E.I.R.L.', 'ASOCIACIÓN DE VIVIENDA VILLA EL MILAGRO MZ. E LOTE 13A ', true, '150142'),
('20609239418', 'ECI TRANSITO & TRANSPORTE SAC', 'AV.  ELIAS  AGUIRRE  115,  MZ.  73,  LT.  11,  CUARTO  PISO,  SECTOR MARIANO MELGAR,  PUEBLO  JOVEN  INCA  PACHACÚTEC', true, '150143'),
('20605144463', 'ESCUELA INTEGRAL & TECNICA DE CONDUCTORES ALIAGA S.A.C.', 'CALLE ELOY URETA N° 112, 2DO. Y 3ER. PISO, MZ.12 LT.12, PP.JJ. CÉSAR VALLEJO', true, '150143'),
('20613077741', 'MI BREVETE PERU S.A.C.', 'AV. PACHACUTEC N° 5101 MZ. A LT. 16 (2DO PISO)', true, '150143'),
('20602001483', 'ESCUELA INTEGRAL DE CONDUCTORES DE TRANSPORTE TERRESTRE JESUS S.A.C.', 'AV. 28 DE JULIO N° 182 PRIMER PISO', true, '150501'),
('20608764977', 'ESCUELA DE CONDUCTORES INTEGRALES CARRETERAS DEL SUR SAC', 'AA.HH. INDUSTRIAS PACOCHA MZ. H, LT. 1, URB. INDUSTRIAS PACOCHA', true, '150801'),
('20605824723', 'LICENCIAS PROFESIONALES PERU S.A.C.', 'A A.VV. LAS GARDENIAS MZ. B, LOTE 11, DEL PROYECTO CIUDAD  SATÉLITE', true, '150801'),
('20600560311', 'SAN CRISTOBAL VIP S.A.C.', 'PAnombreRICANA NORTE Nº 1677, 2DO PISO, ALT. KM 149, DISTRITO DE SANTA MARÍA, PROVINCIA DE HUAURA, DEPARTAMENTO DE LIMA.', true, '150810'),
('20609441993', 'SELVA CAR LORETO S.A.C.', 'CALLE RAMÓN CASTILLA N°325, PISO 2 ', true, '160101'),
('20477455515', 'VEHICULOS PERUANOS S.A.C. - VEHPERU', 'Asent. Humano Rosa Panduro Ramirez Heroina de Roca Fuerte Mz. L Lt. 1', true, '160113'),
('20610844422', 'BREVECAR E.I.R.L.', 'A.H. LA LOMA LAS FLORES, LA CARRETERA III-IV ETAPA MZ. 10B, LOTE 3A', true, '160201'),
('20611061812', 'D&apos;DANNALEX J.M.P. EMPRESA INDIVIDUAL DE RESPONSABILIDAD LIMITADA', 'PASAJE VÍCTOR RAÚL HAYA DE LA TORRE MZ. 6-N,LOTE N° 18-B, AA.HH. HUERTO FAMILIAR (SEGUNDO PISO)', true, '170101'),
('20532825637', 'CORPORACION LADY MANUEL S.A.C.', 'CAL. JUNIN 360', true, '180101'),
('20533055983', 'GRUPO ITEP SOCIEDAD ANONIMA CERRADA - GRUPO ITEP S.A.C.', 'Pampas de San Francisco, Parcela 03, Mz.A Lt.06, 1° Piso,', true, '180101'),
('20532825637', 'CORPORACION LADY MANUEL S.A.C.', 'AA.HH. Programa Municipal de Vivienda II Mz. E Lt. 11', true, '180301'),
('20533055983', 'GRUPO ITEP SOCIEDAD ANONIMA CERRADA - GRUPO ITEP S.A.C.', 'URB.  LUIS  E.  VALCÁRCEL,  MZ.  5,  LOTE  02,  2DO  PISO', true, '180301'),
('20542482029', 'ESCUELA DE CONDUCTORES INTEGRALES MASTER DRIVER S.R.L.', 'JR. WALLER N° 228, LOTE 18 URB. OXAPAMPA', true, '190301'),
('20600510666', 'EL CONDUCTOR RESPONSABLE S.A.C.', 'Av. Luis Antonio Eguiguren Nº 878, 1º y 2º Piso, Urb. La Tina', true, '200101'),
('20526525966', 'ESCUELA DE CONDUCTORES EDUCACION AL VOLANTE S.R.L.', 'Av. Luis Antonio Eguiguren (Ex Malaga) N° 848, Urb. La Tina, 1° y 2° Piso', true, '200101'),
('20482833811', 'ESCUELA DE CONDUCTORES INTEGRALES ALLAIN PROST E.I.R.L.', 'AV. COUNTRY N° 618 –PISO 2 A.H. PACHITEA MZA. J2 LOTE 16B ', true, '200101'),
('20609446863', 'GLOBAL CAR PIURA S.A.C.', 'CL LOS CEIBOS 320 MZ.1 LT.13 PISO 1 URB. STA ISABEL ', true, '200101'),
('20477455515', 'VEHICULOS PERUANOS S.A.C. - VEHPERU', 'Calle Los Ceibos 132 Mz. K Lt. 27 Urb. Santa Isabel ', true, '200101'),
('20605378618', 'SERVICIOS MEDICO DENTAL-SEÑOR DE LUREN S.A.C.', 'ASENTAMIENTO HUMANO MARÍA GORETTI, MZ. D, LT 07, TERCER PISO', true, '200104'),
('20526525966', 'ESCUELA DE CONDUCTORES EDUCACION AL VOLANTE S.R.L.', 'Av Jose de Lama N° 631, 1° y 2° Piso, Urb Jose Idelfonso Coloma', true, '200601'),
('20532712786', 'ECO SUR PERU ESCUELA DE CONDUCTORES DEL SUR SOCIEDAD ANONIMA CERRADA-ECO SUR PERU SAC', 'AV. EL SOL 837 ', true, '210101'),
('20447911371', 'ESCUELA DE CONDUCTORES SAN MARTIN DE PORRES E.I.R.L.', 'JR. AYACUCHO 748', true, '210101'),
('20603304234', 'E.C. INNOVACIONES DE BREVETES DEL SUR S.R.L.', 'CAL. RAMON CASTILLA 622', true, '211101'),
('20610133194', 'ESCUELA DE CONDUCTORES INTEGRALES EL VENCEDOR E.I.R.L.', 'JR. MARIANO MELGAR N° 501', true, '211101'),
('20610844422', 'BREVECAR E.I.R.L.', 'AV. ALMIRANTE GRAU, LOTE 1, SECTOR VILLA HERMOSA, BARRIO EL CALVARIO', true, '220101'),
('20600956061', 'ESCUELA DE CHOFERES INTEGRALES FENIX S.A.C.', 'AV. MAYOR FAP MIGUEL ALEGRE S/N', true, '220601'),
('20600661796', 'NOR CAR PERU ORIENTE E.I.R.L.', 'JR. CUSCO N° 157 BARRIO HUAYCO ', true, '220901'),
('20602005276', 'SELVA CAR SAN MARTIN S.A.C.', 'JR. JIMÉNEZ PIMENTEL N° 1325, 1ER PISO, SECTOR BARRIO HUAYCO', true, '220901'),
('20613859340', 'ASOCIACION EDUCATIVA INTEGRAL KAMBIA SAN MARTIN', 'JR. ANTONIO RAYMONDI N°100 – ESQUINA CON JR. ALFONSO UGARTE N°602', true, '220910'),
('20532712786', 'ECO SUR PERU ESCUELA DE CONDUCTORES DEL SUR SOCIEDAD ANONIMA CERRADA-ECO SUR PERU SAC', 'Av. Saucini S/N, 1° y 2° Piso, ', true, '230101'),
('20532712786', 'ECO SUR PERU ESCUELA DE CONDUCTORES DEL SUR SOCIEDAD ANONIMA CERRADA-ECO SUR PERU SAC', 'Lote 9, Mz. 93, Sector B, Asociación Pro Vivienda de Abogados Guillermo Auza Arce', true, '230101'),
('20532510211', 'ESCUELA DE CONDUCTORES INTEGRALES PERU S.A.C.', 'CAL. ASOC. RICARDO ODONOVAN APV. ASOC. RICARDO ODONOVAN MZA. A LOTE. 17 (ESPALDAS DE DIRECCION REGIONAL DE TRANSP) TACNA TACNA TACNA                        ', true, '230101'),
('20477599193', 'GLOBAL CAR CORP S.A.C.', 'Calle Bolivar N° 878', true, '240101'),
('20477683909', 'SELVA CAR S.A.C.', 'JR. LEONCIO PRADO 125', true, '250101'),
('20477683909', 'SELVA CAR S.A.C.', 'CALLE 4, MZ. 211, LT. 9, SEGUNDO PISO, BARRIO UNIDO 1325', true, '250301');
INSERT INTO peru_centros_medicos (no_ruc, nombre_centro, direccion_centro,estado_autorizacion,id_distrito) VALUES
('20539698282' , 'AUTOESCUELA AQP LA SALLE S.A.C.' , 'AV. KENNEDY 1505' , true , '040112'),
('20311650751' , 'CETPRO PEDRO RUIZ GALLO S.R.L.' , 'URB. CESAR VALLEJO MZ. C, LT. 14-A' , true , '040112'),
('20608763733' , 'ESCUELA DE CONDUCTORES EDUCACION VIAL SAC' , 'MZ.  B  LT.  13, 1ER.  Y  2DO.  PISO, FUNDO  KENNEDY' , true , '040112'),
('20605825673' , 'ESCUELA DE CONDUCTORES INTEGRALES BREVETES ELVIS S.A.C.' , 'AV. GUARDIA CIVIL 2DA FASE, III ETAPA, MZ. B, LT.13 (3ER PISO)' , true , '040112'),
...
--Centros Evaluacion
INSERT INTO peru_centros_evaluacion (no_ruc, nombre_centro, direccion_centro,estado_autorizacion,id_distrito) VALUES
('20148182788' , 'MUNICIPALIDAD PROVINCIAL DE ABANCAY' , 'AV. NUÑEZ S/N' , true, '030101'),
('20408299382' , 'DRTC APURIMAC' , 'AV. MARIÑO N° 125' , true, '030101'),
('20408299382' , 'DRTC APURIMAC' , 'AV. MARINO N°125' , true, '030101'),
('20443070819' , 'DIRECCION SUB REGIONAL DE TRANSPORTES Y COMUNICACIONES CHANKA ANDAHUAYLAS' , 'AV. MANCO CÁPAC 602' , true, '030101'),
('20156959783' , 'MUNICIPALIDAD PROVINCIAL DE ANDAHUAYLAS' , 'JR. RICARDO PALMA 441' , true, '030101'),
('20443070819' , 'DIRECCION SUB REGIONAL DE TRANSPORTES Y COMUNICACIONES CHANKA ANDAHUAYLAS' , 'JR. LEÓNIDAS BERROCAL N° 432' , true, '030101'),
('20165610297' , 'MUNICIPALIDAD PROVINCIAL DE AYMARAES' , 'JR. ANDAHUAYLAS N° 312' , true, '030101'),
('20180001051' , 'MUNICIPALIDAD PROVINCIAL DE CHINCHEROS' , 'JR. RAYMONDI S/N' , true, '030101'),
('20161096068' , 'MUNICIPALIDAD PROVINCIAL COTABAMBAS - APURIMAC' , 'PLAZA DE ARMAS DE TAMBOBAMBA S/N' , true, '030101');
-- MATERIALIZED VIEW PERU UBIGEO
CREATE MATERIALIZED VIEW ubigeo_peru_view AS
SELECT 
	dis.id_distrito,
	dis.nombre_distrito,
	dep.nombre_departamento,
	pro.nombre_provincia
FROM ubigeo_peru_departamentos dep
INNER JOIN ubigeo_peru_provincias pro
	ON dep.id_departamento = pro.id_departamento
INNER JOIN ubigeo_peru_distritos dis
	ON dis.id_provincia = pro.id_provincia;
	
CREATE INDEX idx_ubigeo_mat_distrito ON ubigeo_peru_view(id_distrito);
CREATE INDEX idx_ubigeo_mat_provincia ON ubigeo_peru_view(nombre_provincia);
CREATE INDEX idx_ubigeo_mat_departamento ON ubigeo_peru_view(nombre_departamento);
CREATE INDEX idx_ubigeo_nombre_distrito ON ubigeo_peru_view(nombre_distrito);

CREATE OR REPLACE FUNCTION get_data_by_table_type(
    p_table_type TEXT
)
RETURNS TABLE (
...
BEGIN
    CASE p_table_type
        WHEN 'escuelas' THEN v_table_name := 'peru_driver_escuelas';
        WHEN 'medicos' THEN v_table_name := 'peru_centros_medicos';
        WHEN 'evaluaciones' THEN v_table_name := 'peru_centros_evaluacion';
        ELSE 
            RETURN QUERY SELECT 
                NULL::INTEGER,
                NULL::VARCHAR,
    ...
    v_where_conditions := ' WHERE p.estado_autorizacion = true';
    ...
    v_base_query := '
        SELECT 
            ...
            p.id_distrito  -- ← ADD THIS COLUMN!
        FROM ' || v_table_name || ' p
        LEFT JOIN ubigeo_peru_view v ON p.id_distrito = v.id_distrito' 
        || v_where_conditions;
    
    RETURN QUERY EXECUTE v_base_query;
END;
$$;
CREATE OR REPLACE PROCEDURE insert_centro(
    ...
    id_distrito VARCHAR,
    table_type text,
    INOUT new_id INTEGER DEFAULT NULL
)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    CASE p_table_type
        ...
        WHEN 'medicos' THEN v_table_name := 'peru_centros_medicos';
        WHEN 'evaluaciones' THEN v_table_name := 'peru_centros_evaluacion';
        ELSE RAISE EXCEPTION 'Invalid table type: %', p_table_type;
    END CASE;

    EXECUTE format(
        'INSERT INTO %I (no_ruc, nombre_centro, direccion_centro, email, phone, estado_autorizacion, id_distrito) 
         VALUES ($1, $2, $3, $4, $5, true, $6) RETURNING id_establecimiento',
        v_table_name
    ) USING p_no_ruc, p_nombre_centro, p_direccion_centro, p_email, p_phone, p_id_distrito
    INTO new_id;
END;
$$;

CREATE OR REPLACE PROCEDURE update_centro(
    id_establecimiento INTEGER,
    no_ruc VARCHAR DEFAULT NULL,
    nombre_centro TEXT DEFAULT NULL,
    direccion_centro TEXT DEFAULT NULL,
    email VARCHAR DEFAULT NULL,
    phone VARCHAR DEFAULT NULL,
    id_distrito VARCHAR DEFAULT NULL,
    estado_autorizacion BOOLEAN DEFAULT NULL,
    table_type TEXT
)
LANGUAGE plpgsql
AS $$
...
BEGIN
    ...
    -- Build dynamic SET clauses only for provided parameters
    IF p_no_ruc IS NOT NULL THEN
        param_count := param_count + 1;
        v_conditions := v_conditions || 'no_ruc = $' || param_count || ', ';
        v_params := array_append(v_params, p_no_ruc);
    END IF;

    IF p_nombre_centro IS NOT NULL THEN
        param_count := param_count + 1;
        v_conditions := v_conditions || 'nombre_centro = $' || param_count || ', ';
        v_params := array_append(v_params, p_nombre_centro);
    END IF;

    IF p_direccion_centro IS NOT NULL THEN
        param_count := param_count + 1;
        v_conditions := v_conditions || 'direccion_centro = $' || param_count || ', ';
        v_params := array_append(v_params, p_direccion_centro);
    END IF;

    IF p_email IS NOT NULL THEN
        param_count := param_count + 1;
        v_conditions := v_conditions || 'email = $' || param_count || ', ';
        v_params := array_append(v_params, p_email);
    END IF;

    IF p_phone IS NOT NULL THEN
        param_count := param_count + 1;
        v_conditions := v_conditions || 'phone = $' || param_count || ', ';
        v_params := array_append(v_params, p_phone);
    END IF;

    IF p_id_distrito IS NOT NULL THEN
        param_count := param_count + 1;
        v_conditions := v_conditions || 'id_distrito = $' || param_count || ', ';
        v_params := array_append(v_params, p_id_distrito);
    END IF;

    IF p_estado_autorizacion IS NOT NULL THEN
        param_count := param_count + 1;
        v_conditions := v_conditions || 'estado_autorizacion = $' || param_count || ', ';
        v_params := array_append(v_params, p_estado_autorizacion::TEXT);
    END IF;

    -- Remove trailing comma and space
    IF length(v_conditions) > 0 THEN
        v_conditions := left(v_conditions, length(v_conditions) - 2);
    ELSE
    ...
    param_count := param_count + 1;
    v_params := array_append(v_params, p_id::TEXT);

    -- Build and execute dynamic query
    v_update_query := format(
        'UPDATE %I SET %s WHERE id_establecimiento = $%s',
        v_table_name, v_conditions, param_count
    );

    EXECUTE v_update_query USING v_params;
END;
$$;